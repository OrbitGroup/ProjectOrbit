    ...
    public static class MaxVolumeCalculator
    {
        public static KeyValuePair<Bottlenecks, decimal> GetMaxVolume(Triangle triangle)
        {
            if (triangle.Direction == Directions.BuyBuySell)
            {
                ...
                decimal firstBtcVolume = firstOrderbookLowestAskPrice * triangle.FirstOrderBook[firstOrderbookLowestAskPrice];
                decimal secondBtcVolume = secondOrderbookLowestAskPrice * triangle.SecondOrderBook[secondOrderbookLowestAskPrice] * triangle.FirstOrderBookVolumeConverter.Key;
                // the third direction must be Sell at this point (there is no other potential combination)
                decimal thirdBtcVolume = thirdOrderbookHighestBidPrice * triangle.ThirdOrderBook[thirdOrderbookHighestBidPrice];
            ...
            else if (triangle.Direction == Directions.BuySellSell)
            {
                ...
                decimal firstBtcVolume = firstOrderbookLowestAskPrice * triangle.FirstOrderBook[firstOrderbookLowestAskPrice];
                decimal secondBtcVolume = secondOrderbookHighestBidPrice * triangle.SecondOrderBook[secondOrderbookHighestBidPrice] * thirdOrderbookHighestBidPrice;
            ...
            else //Sell Buy Sell
            {
                ...
                decimal firstBtcVolume = triangle.FirstOrderBook[triangle.FirstOrderBook.Keys.Max()];
                ...
                decimal secondBtcVolume = triangle.SecondOrderBook[triangle.SecondOrderBook.Keys.Min()] * thirdOrderbookhighestBid;
                //the third trade is always in BTC price terms
                decimal thirdBtcVolume = thirdOrderbookhighestBid * triangle.ThirdOrderBook[thirdOrderbookhighestBid];
                //calculate and identify the bottleneck
                if (firstBtcVolume <= secondBtcVolume && firstBtcVolume <= thirdBtcVolume)
                {
                    return new KeyValuePair<Bottlenecks, decimal>(Bottlenecks.FirstTrade, firstBtcVolume);
